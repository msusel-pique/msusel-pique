...
public class RInvoker {
	 ...
	 */
	public static void executeRScript(Script script, Path input, Path output, Path tempResourcesDirectory){
		...
		Path scriptPath = getRScriptResource(script, tempResourcesDirectory).toAbsolutePath();
		...
		Process p = null;
		try {
			p = new ProcessBuilder(rBinPath, scriptPath.toString(), input.toString(), output.toString()).start();
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public static Path getRScriptResource(Script choice, Path outputDirectory) {
